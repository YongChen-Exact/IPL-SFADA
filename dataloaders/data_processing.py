...
with open(r"D:\Datasets\{}\train.txt".format(hopital), "r") as f:
    training_set = [i.replace("\n", "") for i in f.readlines()]
f.close()

with open(r"D:\Datasets\{}\val.txt".format(hopital), "r") as f:
    val_set = [i.replace("\n", "") for i in f.readlines()]
f.close()

with open(r"D:\Datasets\{}\test.txt".format(hopital), "r") as f:
    test_set = [i.replace("\n", "") for i in f.readlines()]
f.close()
...
for case in training_set:
    image = r"D:\Datasets\{}\imagesTr\{}".format(hopital, case)
    label = r"D:\Datasets\{}\labelsTr\{}".format(hopital, case)
    ...
    image_array_recorrected_norm = (
                                               image_array_recorrected - image_array_recorrected.mean()) / image_array_recorrected.std()
    print(image_array.size, "hello")
    for slice_ind in range(image_array.shape[0]):
        f = h5py.File(
            r'D:\Datasets\{}\training_set\{}_slice_{}.h5'.format(hopital, case.replace(".nii.gz", ""), slice_ind), 'w')
        f.create_dataset(
...
val_num = 0
for case in val_set:
    image = "D:\Datasets\{}\imagesTr\{}".format(hopital, case)
    label = "D:\Datasets\{}\labelsTr\{}".format(hopital, case)
    image_itk = sitk.ReadImage(image)
    label_itk = sitk.ReadImage(label)
    ...
    f = h5py.File(r'D:\Datasets\{}\val_set\{}.h5'.format(hopital, case.replace(".nii.gz", "")), 'w')
    f.create_dataset(
...
for case in test_set:
    ...
    f = h5py.File(r'D:\Datasets\{}\test_set\{}.h5'.format(hopital, case.replace(".nii.gz", "")), 'w')
    f.create_dataset(
        'image', data=image_array_recorrected_norm, compression="gzip")
    f.create_dataset('label', data=label_array, compression="gzip")
    f.create_dataset('voxel_spacing', data=image_itk.GetSpacing(), compression="gzip")
